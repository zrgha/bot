...
# GIF added to every message sent by bot
...
GIF_URL = "https://media.giphy.com/media/12bF3AWU423YeA/giphy.gif"
...
def quantity_selector():
    ...
    for qty in range(1, 11):
        ...
        label = f"{qty} (冬{available})" if available > 0 else f"{qty} (❌)"
...
print("Bot is running with GIF enabled...")
last_update_id = 0
...
while True:
    ...
    for update in updates:
        ...
        if "message" in update:
            ...
            if data == "stock":
                stock_text = "冬 Available zeros (accounts):\n"
                for qty in range(1, 11):
                    stock_text += f"冬 {qty} zeros: {stock.get(qty, 0)} available\n"
